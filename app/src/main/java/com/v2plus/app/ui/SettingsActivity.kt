...
import com.v2plus.app.extension.toLongEx
...
import com.v2plus.app.handler.SubscriptionUpdater
import com.v2plus.app.helper.MmkvPreferenceDataStore
...
class SettingsActivity : BaseActivity() {
    ...
    class SettingsFragment : PreferenceFragmentCompat() {
        ...
        private val killSwitch by lazy { findPreference<SwitchPreferenceCompat>(AppConfig.PREF_KILL_SWITCH) }
        private val killSwitchSystem by lazy { findPreference<Preference>(AppConfig.PREF_KILL_SWITCH_SYSTEM) }
        ...
        override fun onCreatePreferences(bundle: Bundle?, s: String?) {
            ...
            preferenceScreen?.let { tintAllIcons(it) }
            ...
            proxySharing?.setOnPreferenceChangeListener { _, _ ->
                true
            }
            ...
            findPreference<SwitchPreferenceCompat>(AppConfig.PREF_VPN_AUTO_PAUSE)?.setOnPreferenceChangeListener { _, newValue ->
                if (newValue == true) {
                    startActivity(Intent(requireContext(), VpnPauseAppsActivity::class.java))
                }
                true
            }
        }
        ...
        override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
            ...
            listView?.addOnChildAttachStateChangeListener(object : RecyclerView.OnChildAttachStateChangeListener {
                override fun onChildViewAttachedToWindow(view: View) {
                    tintSwitchesInView(view)
                }
                override fun onChildViewDetachedFromWindow(view: View) {}
            })
        }

        private fun tintSwitchesInView(view: View) {
            if (view is SwitchCompat) {
                val accent = CustomizationManager.getAccentOpaque()
                val states = arrayOf(
                    intArrayOf(android.R.attr.state_checked),
                    intArrayOf()
                )
                val thumbColors = intArrayOf(accent, Color.parseColor("#888888"))
                val trackColors = intArrayOf(accent and 0x60FFFFFF, Color.parseColor("#444444"))
                view.thumbTintList = ColorStateList(states, thumbColors)
                view.trackTintList = ColorStateList(states, trackColors)
            }
            if (view is ViewGroup) {
                for (i in 0 until view.childCount) {
                    tintSwitchesInView(view.getChildAt(i))
                }
            }
        }

        private fun tintAllIcons(group: PreferenceGroup) {
            ...
            fun updateSummary(pref: androidx.preference.Preference) {
                when (pref) {
                    is EditTextPreference -> {
                        if (pref.key == AppConfig.PREF_LOCAL_PROXY_PASSWORD) {
                            fun refreshPasswordSummary() {
                                val v = pref.text?.trim().orEmpty()
                                pref.summary =
                                    if (v.isEmpty()) pref.context.getString(R.string.summary_pref_local_proxy_password)
                                    else pref.context.getString(R.string.pref_local_proxy_password_masked)
                            }
                            refreshPasswordSummary()
                            pref.setOnPreferenceChangeListener { _, newValue ->
                                val v = (newValue as? String)?.trim().orEmpty()
                                pref.summary = if (v.isEmpty()) {
                                    pref.context.getString(R.string.summary_pref_local_proxy_password)
                                } else {
                                    pref.context.getString(R.string.pref_local_proxy_password_masked)
                                }
                                true
                            }
                        } else {
...
//            localDnsPort?.isEnabled = vpn
            ...
            killSwitch?.isEnabled = vpn
            killSwitchSystem?.isEnabled = vpn
