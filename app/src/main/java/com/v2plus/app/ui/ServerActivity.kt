...
import com.v2plus.app.AppConfig.DEFAULT_PORT
import com.v2plus.app.AppConfig.PREF_ALLOW_INSECURE
import com.v2plus.app.AppConfig.REALITY
import com.v2plus.app.AppConfig.TLS
...
class ServerActivity : BaseActivity() {
    ...
    private val et_uuid: EditText? by lazy { findViewById(R.id.et_uuid) }
    private val sp_congestion_control: Spinner? by lazy { findViewById(R.id.sp_congestion_control) }
    private val sp_udp_relay_mode: Spinner? by lazy { findViewById(R.id.sp_udp_relay_mode) }
    private val cb_zero_rtt_handshake: CheckBox? by lazy { findViewById(R.id.cb_zero_rtt_handshake) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val layoutId = when (config?.configType ?: createConfigType) {
            ...
            EConfigType.TUIC -> R.layout.activity_server_tuic
    ...
    private fun bindingServer(config: ProfileItem): Boolean {
        ...
        if (config.configType == EConfigType.SOCKS || config.configType == EConfigType.HTTP) {
            ...
            et_uuid?.text = Utils.getEditable(config.uuid)
            val congestionControl = Utils.arrayFind(resources.getStringArray(R.array.tuic_congestion_control), config.congestionControl.orEmpty())
            if (congestionControl >= 0) {
                sp_congestion_control?.setSelection(congestionControl)
            }
            val udpRelayMode = Utils.arrayFind(resources.getStringArray(R.array.tuic_udp_relay_mode), config.udpRelayMode.orEmpty())
            if (udpRelayMode >= 0) {
                sp_udp_relay_mode?.setSelection(udpRelayMode)
            }
            cb_zero_rtt_handshake?.isChecked = config.zeroRttHandshake == true
        } else if (config.configType == EConfigType.HYSTERIA || config.configType == EConfigType.HYSTERIA2) {
    ...
    private fun saveServer(): Boolean {
        ...
        if (config.configType != EConfigType.SOCKS
            ...
            if (config.configType == EConfigType.TROJAN
                ...
                || config.configType == EConfigType.HYSTERIA
    ...
    private fun saveCommon(config: ProfileItem) {
        ...
        if (config.configType == EConfigType.VMESS) {
            ...
            config.uuid = et_uuid?.text?.toString()
            config.congestionControl = sp_congestion_control?.selectedItem?.toString()
            config.udpRelayMode = sp_udp_relay_mode?.selectedItem?.toString()
            config.zeroRttHandshake = cb_zero_rtt_handshake?.isChecked
        } else if (config.configType == EConfigType.HYSTERIA || config.configType == EConfigType.HYSTERIA2) {
