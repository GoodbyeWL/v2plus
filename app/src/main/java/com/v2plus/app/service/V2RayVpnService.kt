...
//    }
    ...
    private fun configureVpnService(): Boolean {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q &&
                SettingsManager.isKillSwitchEnabled() &&
                !isLockdownEnabled()
            ) {
                Log.w(
                    AppConfig.TAG,
                    "Kill switch: for full leak protection enable \"Block connections without VPN\" for this app in Android VPN settings."
                )
            }
            return true
        } catch (e: Exception) {
    ...
    private fun configurePerAppProxy(builder: Builder) {
        ...
        val vpnHiddenApps = if (MmkvManager.decodeSettingsBool(AppConfig.PREF_VPN_AUTO_PAUSE, false)) {
