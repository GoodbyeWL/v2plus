...
object AppConfig {
    ...
    /** Synthetic subscription list row (Sub Setting only): profiles with empty subscriptionId; not stored in sub MMKV. */
    const val UNASSIGNED_SUBSCRIPTION_LIST_ID = "__sub_list_unassigned__"
    ...
    /** User expects strict leak protection; pair with system "Block connections without VPN" when true. Default: on. */
    const val PREF_KILL_SWITCH = "pref_killswitch"
    const val PREF_KILL_SWITCH_SYSTEM = "pref_killswitch_system"
    ...
    /** When false, "Ungrouped" is hidden in Sub Setting list and on main tab strip if there are 0 ungrouped profiles. Resets when such profiles appear. */
    const val PREF_SUB_SETTING_SHOW_UNASSIGNED_ROW = "pref_sub_setting_show_unassigned_row"
    ...
    const val PREF_LIVE_SORT_DURING_TEST = "pref_live_sort_during_test"
    const val PREF_COMPATIBILITY_MODE = "pref_compatibility_mode"
    const val PREF_LOCAL_PORT_PROTECTION = "pref_local_port_protection"
    /** Custom SOCKS/HTTP inbound login when [PREF_LOCAL_PORT_PROTECTION] is on; empty = random per session. */
    const val PREF_LOCAL_PROXY_USERNAME = "pref_local_proxy_username"
    /** Custom SOCKS/HTTP inbound password (paired with username). */
    const val PREF_LOCAL_PROXY_PASSWORD = "pref_local_proxy_password"
    const val PREF_SHOW_PROXY_CREDENTIALS = "pref_show_proxy_credentials"
    const val PREF_SHOW_SYSTEM_APPS = "pref_show_system_apps"

    /** VPN auto-pause: temporarily stop VPN when selected apps are in foreground. */
    const val PREF_VPN_AUTO_PAUSE = "pref_vpn_auto_pause"
    const val PREF_VPN_AUTO_PAUSE_APPS = "pref_vpn_auto_pause_apps"

    /** Auto-reconnect: retry current server then failover to another on connection drop. */
    const val PREF_AUTO_RECONNECT = "pref_auto_reconnect"

    /** Cache keys. */
    const val CACHE_SUBSCRIPTION_ID = "cache_subscription_id"
    /**
     * Current SOCKS/HTTP inbound login/password when local proxy protection is active.
     * Written from the VPN daemon process when inbound auth is applied; UI reads via MMKV multi-process.
     */
    const val CACHE_LOCAL_PROXY_SESSION_USER = "cache_local_proxy_session_user"
    const val CACHE_LOCAL_PROXY_SESSION_PASS = "cache_local_proxy_session_pass"
    ...
    const val SUPPORT_URL = "https://t.me/GoodbyeWLALT"
    ...
    const val TG_CHANNEL_URL = SUPPORT_URL
...
//    const val IP_API_URL = "https://speed.cloudflare.com/meta"
    ...
    const val MSG_RELOAD_SERVER_LIST = 80

    /** Notification channel IDs and names. */
    const val RAY_NG_CHANNEL_ID = "RAY_NG_M_CH_ID"
    const val RAY_NG_CHANNEL_NAME = "v2plus Background Service"
