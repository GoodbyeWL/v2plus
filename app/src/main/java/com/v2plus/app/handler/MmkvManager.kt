...
import com.v2plus.app.AppConfig.PREF_IS_BOOTED
import com.v2plus.app.AppConfig.PREF_ROUTING_RULESET
import com.v2plus.app.dto.AssetUrlCache
...
import com.v2plus.app.dto.WebDavConfig
...
object MmkvManager {
    ...
    fun decodeServerConfig(guid: String): ProfileItem? {
        ...
        return try {
            ...
            // Avoid crashing the whole app if a single MMKV entry is corrupted.
            Log.e(AppConfig.TAG, "Invalid server config JSON for guid=$guid", e)
            null
        }
    }
    ...
    /**
     * Gson deserializes missing booleans as false; we treat absent [SubscriptionItem.allowInsecureUrl] as true (product default).
     */
    private fun decodeSubscriptionItemFromJson(json: String): SubscriptionItem {
        val base = JsonUtil.fromJson(json, SubscriptionItem::class.java) ?: SubscriptionItem()
        val obj = JsonUtil.parseString(json) ?: return base
        return if (!obj.has("allowInsecureUrl")) base.copy(allowInsecureUrl = true) else base
    }
    ...
    fun decodeSubscriptions(): List<SubscriptionCache> {
        ...
        decodeSubsList().forEach { key ->
            ...
            if (!json.isNullOrBlank()) {
                val item = decodeSubscriptionItemFromJson(json)
    ...
    fun decodeSubscription(subscriptionId: String): SubscriptionItem? {
        ...
        return decodeSubscriptionItemFromJson(json)
    }
    ...
    fun savePendingUpdate(url: String, version: String) {
        mainStorage.encode(KEY_PENDING_UPDATE_URL, url)
        mainStorage.encode(KEY_PENDING_UPDATE_VERSION, version)
    }

    fun getPendingUpdateUrl(): String? = mainStorage.decodeString(KEY_PENDING_UPDATE_URL)

    fun getPendingUpdateVersion(): String? = mainStorage.decodeString(KEY_PENDING_UPDATE_VERSION)

    fun clearPendingUpdate() {
        mainStorage.remove(KEY_PENDING_UPDATE_URL)
        mainStorage.remove(KEY_PENDING_UPDATE_VERSION)
    }
