...
import com.v2plus.app.AppConfig.DEFAULT_SUBSCRIPTION_ID
import com.v2plus.app.AppConfig.GEOIP_PRIVATE
import com.v2plus.app.AppConfig.GEOSITE_PRIVATE
import com.v2plus.app.AppConfig.TAG_DIRECT
import com.v2plus.app.AppConfig.VPN
...
import com.v2plus.app.enums.RoutingType
import com.v2plus.app.enums.VpnInterfaceAddressConfig
import com.v2plus.app.handler.MmkvManager.decodeAllServerList
import com.v2plus.app.handler.MmkvManager.decodeServerConfig
import com.v2plus.app.handler.MmkvManager.decodeSubsList
import com.v2plus.app.handler.MmkvManager.decodeSubscription
import com.v2plus.app.handler.MmkvManager.encodeSubscription
import com.v2plus.app.handler.MmkvManager.removeSubscription
...
object SettingsManager {
    ...
    fun initApp(context: Context) {
        ...
        migrateLegacyRoutingRulesetsToNewPreset(context)
        migrateServerListToSubscriptions()
        migrateHysteria2PinSHA256()
    }

    private fun migrateLegacyRoutingRulesetsToNewPreset(context: Context) {
        val migrationKey = "routing_ruleset_replace_legacy_with_new_preset"
        ...
        val legacyEnglishRemarks = setOf(
            "Bypass bittorrent",
            "Block udp443",
            "Direct LAN IP",
            "Direct LAN domains",
            "Bypass Russia domains",
            "Bypass Russia IP",
            "Bypass Iran domains",
            "Bypass Iran IP"
        )

        val hasLegacyRules = rulesets.any { item ->
            val remarks = item.remarks.orEmpty()
            remarks.contains(Regex("[\\u4E00-\\u9FFF]")) || legacyEnglishRemarks.contains(remarks)
        }

        if (hasLegacyRules) {
            // Index 1 = WHITE (Авто: РФ напрямую, остальное через прокси)
            val newPreset = getPresetRoutingRulesets(context, 1)
            if (!newPreset.isNullOrEmpty()) {
                MmkvManager.encodeRoutingRulesets(newPreset)
            }
        }
     ...
     * New installs start with no routing rules (all traffic goes through proxy).
     * Existing installs keep their current rulesets unchanged.
     * @param context The application context.
     */
    private fun initRoutingRulesets(context: Context) {
        val initKey = "routing_rulesets_initialized_v2"
        if (MmkvManager.decodeSettingsBool(initKey, false)) {
            return
        }
        val existing = MmkvManager.decodeRoutingRulesets()
        if (existing == null) {
            MmkvManager.encodeRoutingRulesets(mutableListOf())
        }
        MmkvManager.encodeSettings(initKey, true)
    }
     ...
     * @param index The index of the routing type.
     */
    fun resetRoutingRulesetsFromPresets(context: Context, index: Int): Boolean {
        val rulesetList = getPresetRoutingRulesets(context, index) ?: return false
     ...
     * Does not auto-create any fallback subscription group.
     **/
    ...
    fun getLocale(): Locale {
        val langCode = MmkvManager.decodeSettingsString(AppConfig.PREF_LANGUAGE) ?: "ru"
    ...
    /** Strict leak protection intent; combine with system "Block connections without VPN". Default true. */
    fun isKillSwitchEnabled(): Boolean =
        MmkvManager.decodeSettingsBool(AppConfig.PREF_KILL_SWITCH, true)

    /**
     * Ensure default settings are present in MMKV.
     */
    private fun ensureDefaultSettings(context: Context) {
        if (!MmkvManager.decodeSettingsBool("force_vpn_mode_v2", false)) {
            MmkvManager.encodeSettings(AppConfig.PREF_MODE, AppConfig.VPN)
            MmkvManager.encodeSettings("force_vpn_mode_v2", true)
        }
        if (!MmkvManager.decodeSettingsBool("hev_tun_default_disabled_v2", false)) {
            MmkvManager.encodeSettings(AppConfig.PREF_USE_HEV_TUNNEL, false)
            MmkvManager.encodeSettings("hev_tun_default_disabled_v2", true)
        }
        if (!MmkvManager.decodeSettingsBool("pref_compatibility_mode_initialized", false)) {
            // Enable compatibility mode by default on older devices (SDK < 29 or RAM <= 3GB)
            val activityManager = context.getSystemService(Context.ACTIVITY_SERVICE) as android.app.ActivityManager
            val memoryInfo = android.app.ActivityManager.MemoryInfo()
            activityManager.getMemoryInfo(memoryInfo)
            val isLowRam = android.os.Build.VERSION.SDK_INT < 29 || memoryInfo.totalMem <= 3L * 1024 * 1024 * 1024L
            MmkvManager.encodeSettings(AppConfig.PREF_COMPATIBILITY_MODE, isLowRam)
            MmkvManager.encodeSettings("pref_compatibility_mode_initialized", true)
        }
        if (!MmkvManager.decodeSettingsBool("pref_double_column_initialized", false)) {
            ...
            MmkvManager.encodeSettings(AppConfig.PREF_DOUBLE_COLUMN_DISPLAY, isTablet)
            MmkvManager.encodeSettings("pref_double_column_initialized", true)
        }
    ...
    * Servers without subscription are kept under the internal default key.
    ...
    private fun migrateServerListToSubscriptions() {
        ...
        // Group servers by subscription (use internal default key for empty subscriptionId)
        guids.forEach { guid ->
        ...
        // Update each subscription's serverList
