...
import com.v2plus.app.dto.ConfigResult
import com.v2plus.app.dto.ProfileItem
import com.v2plus.app.dto.RulesetItem
import com.v2plus.app.dto.V2rayConfig
import com.v2plus.app.dto.V2rayConfig.OutboundBean
import com.v2plus.app.dto.V2rayConfig.OutboundBean.OutSettingsBean
import com.v2plus.app.dto.V2rayConfig.OutboundBean.StreamSettingsBean
import com.v2plus.app.dto.V2rayConfig.RoutingBean.RulesBean
...
import com.v2plus.app.fmt.HttpFmt
...
object V2rayConfigManager {
    ...
    private fun getV2rayCustomConfig(context: Context, guid: String, config: ProfileItem): ConfigResult {
        ...
        val safe = when (val v = XrayCustomConfigGuard.validateAndSanitize(raw)) {
            is XrayCustomConfigGuard.Result.Ok -> v.json
            is XrayCustomConfigGuard.Result.Rejected -> {
                Log.w(AppConfig.TAG, "Custom config rejected: ${v.reason}")
                return ConfigResult(false)
            }
        }
        val result = ConfigResult(true, guid, safe)
        if (!needTun()) {
            return result
        }

        // check if tun inbound exists
        val json = JsonUtil.parseString(safe) ?: return result
    ...
    private fun getInbounds(v2rayConfig: V2rayConfig): Boolean {
        try {
            ...
            val isProxySharing = MmkvManager.decodeSettingsBool(AppConfig.PREF_PROXY_SHARING) == true
            val isProtectionEnabled = MmkvManager.decodeSettingsBool(AppConfig.PREF_LOCAL_PORT_PROTECTION, false)

            AppConfig.socksUser = null
            AppConfig.socksPass = null

            var persistLocalProxySession = false
            if (!isProxySharing && isProtectionEnabled) {
                val customUser = MmkvManager.decodeSettingsString(AppConfig.PREF_LOCAL_PROXY_USERNAME)?.trim().orEmpty()
                val customPass = MmkvManager.decodeSettingsString(AppConfig.PREF_LOCAL_PROXY_PASSWORD)?.trim().orEmpty()
                val useCustom = customUser.isNotEmpty() && customPass.isNotEmpty()
                val user: String
                val pass: String
                if (useCustom) {
                    user = customUser
                    ...
                    user = "u" + Utils.getUuid().replace("-", "").take(10)
                    pass = Utils.getUuid()
                }
                AppConfig.socksUser = user
                AppConfig.socksPass = pass
                MmkvManager.encodeSettings(AppConfig.CACHE_LOCAL_PROXY_SESSION_USER, user)
                MmkvManager.encodeSettings(AppConfig.CACHE_LOCAL_PROXY_SESSION_PASS, pass)
                persistLocalProxySession = true

                v2rayConfig.inbounds.forEach { inbound ->
                    if (inbound.protocol.equals(EConfigType.SOCKS.name, true)
                        || inbound.protocol.equals(EConfigType.HTTP.name, true)
                    ) {
                        if (inbound.settings == null) {
                            inbound.settings = V2rayConfig.InboundBean.InSettingsBean()
                        }
                        inbound.settings?.auth = "password"
                        inbound.settings?.accounts =
                            listOf(V2rayConfig.InboundBean.InSettingsBean.AccountBean(user, pass))
                    }
                }
            }
            if (!persistLocalProxySession) {
            ...
            if (key.enabled && key.outboundTag == tag && !key.domain.isNullOrEmpty()) {
                key.domain?.forEach {
                    if (it != AppConfig.GEOSITE_PRIVATE
                        ...
                        domain.add(normalizeDomainRule(it))
                    }
                }
            }
        }
    ...
    /**
     * Core expects geosite/geoip codes in the same form as in the dat file (typically lowercase).
     * User/imported rules sometimes contain `geosite:CN` which will fail with:
     * `code not found in geosite.dat: CN`.
     */
    private fun normalizeDomainRule(rule: String): String {
        val trimmed = rule.trim()
        if (!trimmed.startsWith("geosite:", ignoreCase = true)) return trimmed
        val code = trimmed.substringAfter("geosite:", missingDelimiterValue = "").trim()
        if (code.isEmpty()) return trimmed
        return "geosite:${code.lowercase()}"
    }
    ...
    private fun getMoreOutbounds(v2rayConfig: V2rayConfig, subscriptionId: String): Boolean {
        ...
        try {
            ...
            val globalExitNodeId = MmkvManager.decodeSettingsString("pref_global_exit_node", "") ?: ""
            val isBridgeEnabled = MmkvManager.decodeSettingsBool("pref_bridge_enabled", false)
            
            if (isBridgeEnabled && globalExitNodeId.startsWith("auto:")) {
                val filterId = globalExitNodeId.substring(5)
                val allServers = MmkvManager.decodeAllServerList().mapNotNull { MmkvManager.decodeServerConfig(it) }
                
                val filtered = if (filterId == "all") {
                    allServers
                } else if (filterId == "") {
                    allServers.filter { it.subscriptionId.isEmpty() }
                } else if (filterId.startsWith("group:")) {
                    val gName = filterId.substring(6)
                    val subIds = MmkvManager.decodeSubscriptions().filter { it.subscription.group == gName }.map { it.guid }
                    allServers.filter { subIds.contains(it.subscriptionId) }
                } else {
                    allServers.filter { it.subscriptionId == filterId }
                }
                
                // Pick best candidates (standard web ports usually don't get blocked) and limit to max 15 to avoid config bloat
                var bestServers = filtered.filter { 
                    val p = it.serverPort?.toIntOrNull() ?: 0
                    p == 443 || p == 80 || p == 8443 || p == 2053 || p == 8080 
                }
                if (bestServers.isEmpty()) bestServers = filtered // fallback to all if none match
                bestServers = bestServers.take(15)
                
                if (bestServers.isNotEmpty()) {
                    outbound.tag = AppConfig.TAG_PROXY + "1" // Entry node becomes proxy1
                    outbound.mux?.enabled = false
                    
                    val exitTags = mutableListOf<String>()
                    
                    bestServers.forEachIndexed { index, nextNode ->
                        if (nextNode.server != outbound.getServerAddress()) {
                            val nextOutbound = convertProfile2Outbound(nextNode)
                            if (nextOutbound != null) {
                                updateOutboundWithGlobalSettings(nextOutbound)
                                nextOutbound.tag = "bridge_exit_$index"
                                nextOutbound.mux?.enabled = false
                                nextOutbound.ensureSockopt().dialerProxy = outbound.tag
                                v2rayConfig.outbounds.add(0, nextOutbound)
                                exitTags.add(nextOutbound.tag)
                            }
                        }
                    }
                    
                    if (exitTags.isNotEmpty()) {
                        val balancer = V2rayConfig.RoutingBean.BalancerBean(
                            tag = "bridge_balancer",
                            selector = listOf("bridge_exit_"),
                            strategy = V2rayConfig.RoutingBean.StrategyObject(type = "leastPing")
                        )
                        
                        v2rayConfig.observatory = V2rayConfig.ObservatoryObject(
                            subjectSelector = listOf("bridge_exit_"),
                            probeUrl = MmkvManager.decodeSettingsString(AppConfig.PREF_DELAY_TEST_URL) ?: AppConfig.DELAY_TEST_URL,
                            probeInterval = "3m",
                            enableConcurrency = true
                        )
                        
                        if (v2rayConfig.routing.balancers == null) {
                            v2rayConfig.routing.balancers = mutableListOf()
                        }
                        (v2rayConfig.routing.balancers as MutableList).add(balancer)
                        
                        // We will replace rule.outboundTag == "proxy" with balancer in getBalance or here
                        v2rayConfig.routing.rules.forEach { rule ->
                            if (rule.outboundTag == "proxy" || rule.outboundTag == AppConfig.TAG_PROXY) {
                                rule.outboundTag = null
                                rule.balancerTag = "bridge_balancer"
                            }
                        }
                    } else {
                        // Revert tag if no valid exit nodes
                        outbound.tag = AppConfig.TAG_PROXY
                    }
                }
            } else {
                var nextNode: com.v2plus.app.dto.ProfileItem? = null
                if (isBridgeEnabled && globalExitNodeId.isNotEmpty()) {
                    nextNode = MmkvManager.decodeServerConfig(globalExitNodeId)
                }

                val subItem = if (subscriptionId.isNotEmpty()) MmkvManager.decodeSubscription(subscriptionId) else null
                
                if (subItem != null) {
                    //Previous proxy
                    val prevNode = SettingsManager.getServerViaRemarks(subItem.prevProfile)
                    if (prevNode != null && prevNode.server != outbound.getServerAddress()) {
                        ...
                        if (prevOutbound != null) {
                            ...
                            outbound.mux?.enabled = false
                            prevOutbound.mux?.enabled = false
                    ...
                    if (nextNode == null) {
                        nextNode = SettingsManager.getServerViaRemarks(subItem.nextProfile)
                    }
                }

                //Next proxy
                if (nextNode != null && nextNode.server != outbound.getServerAddress()) {
                    val nextOutbound = convertProfile2Outbound(nextNode)
                    if (nextOutbound != null) {
                        updateOutboundWithGlobalSettings(nextOutbound)
                        nextOutbound.tag = AppConfig.TAG_PROXY
                        
                        // Disable mux for proxy chains to avoid multiplexing issues over chained connections
                        outbound.mux?.enabled = false
                        nextOutbound.mux?.enabled = false
    ...
    private fun resolveOutboundDomainsToHosts(v2rayConfig: V2rayConfig) {
        ...
        for (item in proxyOutboundList) {
            ...
            // If the outbound is routed through another proxy (like Bridge Exit Node),
            // DO NOT resolve its domain locally to prevent DNS poisoning and let the Entry Node resolve it.
            if (!item.streamSettings?.sockopt?.dialerProxy.isNullOrEmpty()) continue
    ...
    private fun convertProfile2Outbound(profileItem: ProfileItem): OutboundBean? {
        return when (profileItem.configType) {
            ...
            EConfigType.TUIC -> TuicFmt.toOutbound(profileItem)
            EConfigType.HYSTERIA -> HysteriaFmt.toOutbound(profileItem)
     ...
     */
    /**
     * Xray expects bandwidth like "100 mbps". Plain numbers are normalized; empty/0 omits brutal and uses core defaults.
     */
    private fun normalizeHysteriaBandwidthForCore(raw: String?): String? {
        val t = raw?.trim()?.takeIf { it.isNotEmpty() } ?: return null
        if (t == "0") return null
        if (t.contains("bps", ignoreCase = true)) return t
        return if (Regex("^\\d+(\\.\\d+)?$").matches(t)) "$t mbps" else t
    }
...
//        val key = profileItem.quicKey
        ...
        when (streamSettings.network) {
            NetworkType.TCP.type -> {
                ...
                if (headerType == AppConfig.HEADER_TYPE_HTTP) {
                    ...
                    if (!TextUtils.isEmpty(host) || !TextUtils.isEmpty(path)) {
                        ...
                        requestObj.headers.hwid = listOf(Utils.getHwid())
            ...
            NetworkType.WS.type -> {
                ...
                wssetting.headers.hwid = Utils.getHwid()
                sni = host
                wssetting.path = path ?: "/"
...
//                    }
            ...
            NetworkType.HYSTERIA.type -> {
                val hyVersion = when (profileItem.configType) {
                    EConfigType.HYSTERIA -> 1
                    else -> 2
                }
                val hysteriaSetting = StreamSettingsBean.HysteriaSettingsBean(
                    version = hyVersion,
                    auth = profileItem.password.orEmpty(),
                    up = normalizeHysteriaBandwidthForCore(profileItem.bandwidthUp),
                    down = normalizeHysteriaBandwidthForCore(profileItem.bandwidthDown),
                    udphop = null
                )
