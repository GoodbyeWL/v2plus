...
open class SimpleItemTouchHelperCallback(private val mAdapter: ItemTouchHelperAdapter, private val allowSwipe: Boolean = false) : ItemTouchHelper.Callback() {
    ...
    /**
     * Override in subclasses to lock specific rows (e.g. synthetic header).
     */
    protected open fun isDragAllowed(viewHolder: RecyclerView.ViewHolder): Boolean = true

    override fun getMovementFlags(
        recyclerView: RecyclerView,
        viewHolder: RecyclerView.ViewHolder
    ): Int {
        if (!isDragAllowed(viewHolder)) {
