...
object Utils {
     ...
     */
    fun isXray(): Boolean = BuildConfig.APPLICATION_ID.startsWith("com.v2plus.app")
    ...
    fun getHwid(): String {
        var hwid = MmkvManager.decodeSettingsString("app_hwid")
        if (!hwid.isNullOrEmpty()) {
            return hwid
        }
        try {
            hwid = Settings.Secure.getString(com.v2plus.app.AngApplication.application.contentResolver, Settings.Secure.ANDROID_ID)
        } catch (e: Exception) {
        }
        if (hwid.isNullOrEmpty() || hwid == "unknown") {
            hwid = UUID.randomUUID().toString().replace("-", "").substring(0, 16)
        }
        MmkvManager.encodeSettings("app_hwid", hwid)
        return hwid!!
    }
